...
<%@ page import="com.sandeep.modals.DAO" %>
	...
	<body>
	  ...
	  <nav class="navbar navbar-expand-sm container my-3">
	    ...
	    <div class="collapse navbar-collapse" id="my-navbar">
	      ...
	      <label class="bg-primary text-white p-1 rounded mt-2" > Welcome: <b><%= name %></b>
	      </label>
	      <a class=" bg-danger text-white p-1 rounded " href="LogOut"> LogOut </a> 
	 ...
	 <% 
	    ...
	    db.closeConnection();
	    for(HashMap<String,String> sbj: enquiries)
	  {	
	    	 %>
	    <p class="bg-warning p-3 m-2">
	   
	    	    ID:<b><%=sbj.get("id") %></b>
	    	    Name:<b><%=sbj.get("name") %></b>
	    	    Phone:<b><%=sbj.get("phone") %></b>
	    	    Status:<b><%=sbj.get("status") %></b>
	    	    <% 
	    	    String status=(String)sbj.get("status");
	    	    if(status.equalsIgnoreCase("pending")){
	    	    	
	    	    %>
	    	 <a class="btn btn-sm btn-danger" href="ChangeEnquiryStatus?id=<%=sbj.get("id")%>">Done</a>
